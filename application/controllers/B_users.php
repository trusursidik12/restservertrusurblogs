...
class B_users extends CI_Controller {
	...
	{
		$data['users'] 			= $this->b_users_m->get_users();
		$data['usersinfo'] 		= $this->b_users_m->get_usersinfo();
		$data['levels'] 		= $this->b_users_m->get_levels();
		$data['title_header'] 	= "Users List";
		$data['title_menu'] 	= "Add User";
		$data['controllers'] 	= "users";
		
		$this->temp_backend->load('backend/theme/template', 'backend/users/users_list', $data);
	}
	...
	public function add(){
		...
		if($this->form_validation->run() === FALSE){
			$this->temp_backend->load('backend/theme/template', 'backend/users/users_form_add', $data);
		} else {
			$config['upload_path'] = './assets/img/accounts/users';
			...
			$this->b_users_m->add_users($photo);
			redirect('backoffice/users/list');
		}
	}

	public function edit($slug = NULL){
		$data['users'] 			= $this->b_users_m->get_usersview($slug);
		$data['levels'] 		= $this->b_users_m->get_levels();
		...
		if($this->form_validation->run() === FALSE){
			$this->temp_backend->load('backend/theme/template', 'backend/users/users_form_edit', $data);
		} else {
			...
			if(!$this->upload->do_upload('usr_photo')){
				...
				$data = $this->b_users_m->ambil_id_gambar($id);
				$path = './assets/img/accounts/users/';
			...
			$this->b_users_m->update_users($photo);
			redirect('backoffice/users/list');
		}
	}
	...
	public function delete($id){
		$id 	= decrypt_url($id);
		$data 	= $this->b_users_m->ambil_id_gambar($id);//ambil id gambar	  
		$path 	= './assets/img/accounts/users/';// lokasi gambar berada
		unlink($path.$data->usr_photo);// hapus data di folder dimana data tersimpan
		$this->b_users_m->delete_users($id);
		redirect('backoffice/users/list');
	}
}
